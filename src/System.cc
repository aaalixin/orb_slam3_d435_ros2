...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
    ...
    // 添加 ROS 相关成员初始化
    ros_node_(nullptr),
    map_pointcloud_pub_(nullptr),
    current_pointcloud_pub_(nullptr),
    camera_pose_pub_(nullptr),
...
Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    // std::cout << "out grabber" << std::endl;
        if(ros_node_ && !mbShutDown)
    {
        PublishCameraPose(Tcw, timestamp);
    }
...
Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
        ...
        if(ros_node_ && !mbShutDown)
    {
        ...
        // 发布当前帧点云
        PublishCurrentPointCloud(imToFeed, imDepthToFeed, timestamp);
        
        // 每10帧发布一次地图点云
        static int map_publish_count = 0;
        if(++map_publish_count >= 10)
        {
...
void System::SetROSNode(std::shared_ptr<rclcpp::Node> node)
{
    ros_node_ = node;
    if(ros_node_) {
        // 创建点云发布器
        map_pointcloud_pub_ = ros_node_->create_publisher<sensor_msgs::msg::PointCloud2>("/d435/map_points", 10);
        current_pointcloud_pub_ = ros_node_->create_publisher<sensor_msgs::msg::PointCloud2>("/d435/current_points", 10);
        
        // 创建相机位姿和轨迹发布器
        camera_pose_pub_ = ros_node_->create_publisher<geometry_msgs::msg::PoseStamped>("/d435/camera_pose", 10);
        trajectory_pub_ = ros_node_->create_publisher<nav_msgs::msg::Path>("/d435/trajectory", 10);
        
        // 初始化轨迹
        trajectory_path_.header.frame_id = "map";
        
        // 输出日志
        std::cout << "ORB-SLAM3: ROS node set, all publishers created" << std::endl;
        std::cout << "Point cloud topics: /d435/map_points, /d435/current_points" << std::endl;
        std::cout << "Pose topics: /d435/camera_pose, /d435/trajectory" << std::endl;
    }
}

void System::PublishMapPointCloud()
{
    if(!ros_node_ || !mpAtlas || mbShutDown) return;
    ...
    // 获取所有地图点
    std::vector<MapPoint*> mapPoints = currentMap->GetAllMapPoints();
    ...
    for(MapPoint* pMP : mapPoints)
    {
        if(pMP && !pMP->isBad())
        {
            ...
            if(observations > 10) {
                colors.push_back(cv::Vec3b(0, 255, 0)); // 绿色 - 高质量点
            } else {
                colors.push_back(cv::Vec3b(255, 0, 0)); // 红色 - 普通点
            }
        }
    }
    ...
    if(!points.empty())
    {
        auto msg = PointCloudToROSMsg(points, colors);
        map_pointcloud_pub_->publish(msg);
        
        // 可选：输出调试信息
        static int count = 0;
        if(++count % 50 == 0) {
            std::cout << "ORB-SLAM3" << points.size() << " points" << std::endl;
        }
    }
}

void System::PublishCurrentPointCloud(const cv::Mat& imRGB, const cv::Mat& imDepth, double timestamp)
{
    if(!ros_node_ || !mpTracker || mbShutDown) return;
    ...
    for(int i = 0; i < currentFrame.N; i++)
    {
        ...
        if(pMP && !pMP->isBad())
        {
            ...
            {
                colors.push_back(cv::Vec3b(0, 255, 255)); // 黄色 - 默认颜色
            }
        }
    }
    ...
    {
        auto msg = PointCloudToROSMsg(points, colors);
        current_pointcloud_pub_->publish(msg);
    }
}

sensor_msgs::msg::PointCloud2 System::PointCloudToROSMsg(const std::vector<Eigen::Vector3f>& points, 
                                                       const std::vector<cv::Vec3b>& colors)
{
    ...
    for(size_t i = 0; i < points.size(); ++i)
    {
        ...
        float_ptr[3] = *reinterpret_cast<float*>(&rgb_packed);
    }
    
    return msg;
}
void System::PublishCameraPose(const Sophus::SE3f& Tcw, double timestamp)
{
    ...
    // 更新并发布轨迹
    trajectory_path_.header.stamp = ros_node_->now();
